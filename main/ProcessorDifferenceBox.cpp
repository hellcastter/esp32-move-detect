...
const char* PDBoxTag = "ProcessorDifferenceBoxTAG";

ProcessorDifferenceBox::ProcessorDifferenceBox(Camera *camera) {
    cam = camera;
    ...
    ESP_LOGI(PDBoxTag, "ProcessorDifferenceBox was inited");

    cam->free_picture();
}


ProcessorDifferenceBox::~ProcessorDifferenceBox() {
    delete[] prev;
    delete[] same;
}

camera_fb_t* ProcessorDifferenceBox::iterate() {
    auto fb = cam->take_picture();
...
Rect ProcessorDifferenceBox::dfs(size_t orPos) {
    std::queue<size_t> q;
    q.push(orPos);
...
std::vector<Rect> ProcessorDifferenceBox::merge_rects(std::vector<Rect> rects) {
...
void ProcessorDifferenceBox::draw(const std::vector<Rect> &rects, camera_fb_t *fb) {
    for (auto rect : rects) {
