...
const char* MedBoxTAG = "ProcessorMedianBoxTAG";

ProcessorMedianBox::ProcessorMedianBox(Camera* camera){
    cam = camera;
    ...
    ESP_LOGI(MedBoxTAG, "Processor was initialized");
}

ProcessorMedianBox::~ProcessorMedianBox() {
    for (size_t iter = 0; iter < LAST_FRAMES_NUM; iter++){
        free(prev[iter]);
    }
    free(same);
...
uint8_t ProcessorMedianBox::medianValue(size_t pixIdx){
    for(size_t iter = 0; iter < LAST_FRAMES_NUM; iter++){
        size_t min = 0;
        size_t max = 0;
...
camera_fb_t* ProcessorMedianBox::iterate(){
    auto fb = cam->take_picture();
...
Rect ProcessorMedianBox::dfs(size_t orPos) {
    std::queue<size_t> q;
    q.push(orPos);
...
std::vector<Rect> ProcessorMedianBox::merge_rects(std::vector<Rect> rects) {
...
void ProcessorMedianBox::draw(const std::vector<Rect> &rects, camera_fb_t *fb) {
    for (auto rect : rects) {
